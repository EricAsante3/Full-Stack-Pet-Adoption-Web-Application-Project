...
const CardContainer: React.FC<CardContainerProps> = ({ filter }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5170/fetch_favorited_pets", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({"user_id": sessionStorage.getItem("userId")}), // Convert payload to JSON string
        });
